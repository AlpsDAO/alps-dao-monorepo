...
import { AlpsDAOLogicV2, AlpsDescriptorV2__factory, AlpsToken } from '../../../../typechain';
...
describe('Vote Refund', () => {
  before(async () => {
    ...
    token = await deployAlpsToken(deployer);
    const descriptor = AlpsDescriptorV2__factory.connect(await token.descriptor(), deployer);
