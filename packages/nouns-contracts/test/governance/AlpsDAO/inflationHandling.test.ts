...
import { AlpsToken, AlpsDAOLogicV1 } from '../../../typechain';
...
describe('AlpsDAO#inflationHandling', () => {
  before(async () => {
  ...
  it('returns quorum votes and proposal threshold based on Alp total supply', async () => {
    // Total Supply = 40
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    ).revertedWith('AlpsDAO::propose: proposer votes below proposal threshold');
  });
