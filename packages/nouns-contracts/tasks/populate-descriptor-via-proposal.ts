...
task(
  ...
  'Populates the descriptor with color palettes and Alp parts; accepts an input JSON with missing properies.',
)
  .addParam('alpsDescriptor', 'The `AlpsDescriptor` contract address')
  .addParam('daoAddress', 'The `AlpsDAOProxy` contract address')
  ...
  .setAction(
    async ({ alpsDescriptor, daoAddress, imageDataPath, proposalTextPath }, { ethers }) => {
      ...
      const dao = (await ethers.getContractFactory('AlpsDAOLogicV1')).attach(daoAddress);
