...
task('deploy-test-token', 'Deploy AlpsTokenHarness given a descriptor')
  ...
  .setAction(async ({ descriptorAddress, seederAddress }, { ethers, run, network }) => {
    ...
    const token = await (
      await ethers.getContractFactory('AlpsTokenHarness', deployer)
    ).deploy(
      deployer.address,
      deployer.address,
      descriptorAddress,
    ...
    console.log(`AlpsTokenHarness deployed to: ${token.address}`);
    ...
    if (network.name !== 'localhost') {
      ...
      contracts.AlpsTokenHarness = {
        name: 'AlpsTokenHarness',
