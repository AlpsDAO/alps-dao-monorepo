...
import AlpImageVoteTable from '../AlpImageVoteTable';
import VoteProgressBar from '../VoteProgressBar';
import classes from './VoteCard.module.css';
import { Trans } from '@lingui/macro';
import { i18n } from '@lingui/core';
import DelegateGroupedAlpImageVoteTable from '../DelegateGroupedAlpImageVoteTable';
...
const VoteCard: React.FC<VoteCardProps> = props => {
  const { proposal, percentage, alpIds, variant, delegateView, delegateGroupedVoteData } = props;
  ...
  useEffect(() => {
    if (!delegateGroupedVoteData || ensCached) {
      return;
    }
    ...
    delegateGroupedVoteData.forEach((delegateInfo: { delegate: string }) => {
      ...
      lookupNNSOrENS(publicProvider, delegateInfo.delegate)
        .then(name => {
    ...
    setEnsCached(true);
  }, [ensCached, delegateGroupedVoteData]);
    ...
    <Col lg={4} className={classes.wrapper}>
      <Card className={classes.voteCountCard}>
        <Card.Body className="p-2">
          ...
          <Row className={classes.alpProfilePics}>
            {delegateView ? (
              <DelegateGroupedAlpImageVoteTable
              ...
              <AlpImageVoteTable alpIds={alpIds} propId={parseInt(proposal.id || '0')} />
            )}
          </Row>
        </Card.Body>
      </Card>
