...
import { ProposalState } from '../../wrappers/alpsDao';

import classes from './AlpProfileVoteRow.module.css';
...
import { AlpVoteHistory } from '../ProfileActivityFeed';
...
const selectIconForAlpVoteActivityRow = (proposal: Proposal, vote?: AlpVoteHistory) => {
  if (!vote) {
...
const selectVotingInfoText = (proposal: Proposal, vote?: AlpVoteHistory) => {
  if (!vote) {
    if (proposal.status === ProposalState.PENDING || proposal.status === ProposalState.ACTIVE) {
...
const AlpProfileVoteRow: React.FC<AlpProfileVoteRowProps> = props => {
  ...
  return (
    <tr onClick={proposalOnClickHandler} className={classes.voteInfoRow}>
      <td className={classes.voteIcon}>{selectIconForAlpVoteActivityRow(proposal, vote)}</td>
...
export default AlpProfileVoteRow;
