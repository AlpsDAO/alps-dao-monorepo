import { Proposal, ProposalState, useProposalThreshold } from '../../wrappers/alpsDao';
...
const Proposals = ({ proposals }: { proposals: Proposal[] }) => {
  ...
  const hasEnoughVotesToPropose = account !== undefined && connectedAccountAlpVotes >= threshold;
  const hasAlpBalance = (useUserAlpTokenBalance() ?? 0) > 0;

  const nullStateCopy = () => {
    if (account !== null) {
      if (connectedAccountAlpVotes > 0) {
    ...
    <div className={classes.proposals}>
      ...
      >
        ...
        {hasEnoughVotesToPropose ? (
          <div className={classes.alpInWalletBtnWrapper}>
